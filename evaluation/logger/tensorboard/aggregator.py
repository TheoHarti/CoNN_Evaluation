...
if __name__ == '__main__':
    ...
    base_path = "D:\Projects\Masterthesis\MT_Implementations\GNN_Evaluation\logs\Spirals\CasCor"
    parser = argparse.ArgumentParser()
    parser.add_argument("--path", type=str, help="main path for tensorboard files", default=base_path)
    parser.add_argument("--subpaths", type=param_list, help="subpath structures", default=['.'])
    parser.add_argument("--output", type=str, help="aggregation can be saved as tensorboard file (summary) or as table (csv)", default='csv')

    args = parser.parse_args()

    hp_paths = ["\\0.05\\15", "\\0.1\\15", "\\0.3\\15", "\\0.05\\30", "\\0.1\\30", "\\0.3\\30"]
    for hp_path in hp_paths:
        cur_path = base_path + hp_path
        for datetime in os.scandir(cur_path):
            full_path = cur_path + "\\" + datetime.name
